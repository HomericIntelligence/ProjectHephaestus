...
_No response_
END_5CB227EAEAA5242E_TASK

**Task review — the plan-review verdict (untrusted):**
BEGIN_5CB227EAEAA5242E_TASK_REVIEW
## 🔍 Plan Review
...
Grade: A
Verdict: GO
END_5CB227EAEAA5242E_TASK_REVIEW

**Current implementation diff (untrusted):**
BEGIN_5CB227EAEAA5242E_DIFF
...
index 9fa57115..dc3a7b40 100644
...
@@ -647,14 +648,11 @@ def _run_codex_command(
...
@@ -678,9 +676,11 @@ def _pi_sandbox_args(sandbox: str) -> list[str]:
...
+        env[PI_MODEL_ENV] = model
     ...
     return env
@@ -693,6 +693,7 @@ def _run_pi_command(
     cwd: Path,
     timeout: int,
     sandbox: str,
+    model: str = "",
...
@@ -773,13 +774,14 @@ def run_pi_session(
 ) -> AgentRunResult:
...
@@ -795,13 +797,14 @@ def resume_pi_session(
     model: str = "",
 ) -> AgentRunResult:
...
index 8ee7f034..a2082beb 100644
...
@@ -32,24 +38,59 @@ def build_parser() -> argparse.ArgumentParser:
...
@@ -59,9 +100,11 @@ def main(argv: list[str] | None = None) -> int:
...
index d4f0c0cb..2d061cc8 100644
...
END_5CB227EAEAA5242E_DIFF


**Review Threads to Address (untrusted):**
BEGIN_5CB227EAEAA5242E_THREADS_JSON
[{"thread_id": "PRRT_kwDOQww0as6Mwsbf", "path": "scripts/pi_smoke.py", "line": 90, "body": "`run_pi_session()` is called without `model=model`. Line 87 reads the alias from `PI_MODEL_ENV` into `model`, but it is never forwarded. Since `_pi_env(model=model)` in the runtime is the intended propagation path, omitting `model=model` here means the runtime's `_pi_env` receives an empty string. Fix: add `model=model` to the `run_pi_session(...)` call."}, {"thread_id": "PRRT_kwDOQww0as6Mwsbh", "path": "scripts/pi_smoke_slurm.py", "line": 30, "body": "`_missing_alias_env()` and `REQUIRED_ALIAS_ENVS` are byte-for-byte duplicates of `scripts/pi_smoke.py:50-52` and `scripts/pi_smoke.py:32`. Both scripts already import from `hephaestus.agents.runtime`; move the shared constant and helper there (or a shared private utility) to avoid divergence."}]
END_5CB227EAEAA5242E_THREADS_JSON
...
BEGIN_5CB227EAEAA5242E_TODO_LIST
@ scripts/pi_smoke.py Line 90 - simple - `run_pi_session()` is called without `model=model`. Line 87 reads the alias from `PI_MODEL_ENV` into `model`, but it is never forwarded. Since `_pi_env(model=model)` in the runtime is the intended pro…
@ scripts/pi_smoke_slurm.py Line 30 - medium - `_missing_alias_env()` and `REQUIRED_ALIAS_ENVS` are byte-for-byte duplicates of `scripts/pi_smoke.py:50-52` and `scripts/pi_smoke.py:32`. Both scripts already import from `hephaestus.agents.runtime`;…
END_5CB227EAEAA5242E_TODO_LIST
