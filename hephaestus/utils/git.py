"""Shared Git subprocess helpers for repository operations.
...
_NETWORK_GIT_COMMANDS: frozenset[str] = frozenset({"clone", "fetch", "push", "pull", "ls-remote"})
...
def _strip_git_prefix(args: list[str]) -> list[str]:
    """Return git command arguments without a leading ``git`` program name."""
    return args[1:] if args and args[0] == "git" else args
...
def run_git(
    ...
    *,
    ...
    """Run a Git command with bounded timeout and optional retry protection.

    Args:
        args: Git arguments, with or without a leading ``git``.
        cwd: Working directory for the command.
        dry_run: If True, log the command and return a successful result.
        check: Whether to raise on non-zero exit.
        timeout: Explicit timeout. Defaults to network timeout for network Git
            subcommands and metadata timeout otherwise.
        retries: Number of retry attempts for transient subprocess failures.

    Returns:
        CompletedProcess instance.

    """
    git_args = _strip_git_prefix(args)
    cmd = ["git", *git_args]
    effective_timeout = timeout or (
        NETWORK_TIMEOUT if git_args and git_args[0] in _NETWORK_GIT_COMMANDS else METADATA_TIMEOUT
    )

    if dry_run:
        suffix = f" (in {cwd})" if cwd else ""
        logger.info("[dry-run] git %s%s", " ".join(git_args), suffix)
        return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")

    def _call() -> subprocess.CompletedProcess[str]:
        return run(cmd, cwd=cwd, check=check, timeout=effective_timeout)
...
def git_config_get(
    ...
    *,
    ...
    result = run_git(
        ...
        cwd=cwd,
        check=False,
        timeout=timeout or METADATA_TIMEOUT,
    )
...
def git_branch_exists(branch_name: str, *, cwd: Path | None = None) -> bool:
    """Return True if ``branch_name`` exists as a local branch."""
    try:
        result = run_git(
...
def git_rev_parse(
    ...
    *,
    ...
    """Run ``git rev-parse`` with caller-supplied arguments."""
    return run_git(
        ["rev-parse", *args],
        cwd=cwd,
        check=check,
...
def git_show_toplevel(
    *,
    ...
    """Return the top-level path for the current Git repository."""
...
def git_diff_unmerged_names(
    cwd: Path,
    *,
    check: bool = True,
    timeout: int | None = None,
) -> list[str]:
    """Return names of files currently marked unmerged by Git."""
    result = run_git(
...
def git_ls_remote(
    ...
    *,
    ...
    """Run ``git ls-remote`` for ``remote`` and ``ref``."""
    return run_git(
...
def git_fetch(
    ...
    *,
    ...
    """Fetch a remote, optionally narrowing to one ref."""
    args = ["fetch", remote]
    if ref:
        args.append(ref)
    return run_git(
...
def git_rebase(
    ...
    *,
    ...
    """Run ``git rebase`` against ``upstream``, optionally with ``--exec``."""
    args = ["rebase", upstream]
    if exec_cmd is not None:
        args.extend(["--exec", exec_cmd])
    return run_git(
        args,
        cwd=cwd,
        dry_run=dry_run,
