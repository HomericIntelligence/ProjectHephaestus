...
- States: ENTER -> GATE -> WORKTREE_WAIT -> DIRTY_DECISION_WAIT -> DIRTY_RECOVERY_WAIT ->
  ADVISE_WAIT -> IMPLEMENT_WAIT -> REBASE_CONTINUE_WAIT /
  REBASE_CONFLICT_WAIT -> REBASE_CONFLICT_VALIDATE_WAIT -> TEST_WAIT -> TESTFIX_WAIT ->
...
- Prompt functions (imported, never re-authored):
  ...
  Rebase conflicts use ``get_rebase_conflict_prompt`` with host-provided
  bounded hunk context.
"""
...
class ImplementationStage(Stage):
    ...
    def _rebase_continue_wait(self, item: WorkItem, ctx: StageContext) -> StepResult:
        ...
        if item.payload.pop("rebase_complete", None):
            ...
            item.payload.pop("rebase_conflict_hunks", None)
            ...
            item.payload.pop(_REBASE_CONFLICT_RESOLUTION_KEY, None)
            item.payload.pop(_REBASE_CONFLICT_DIAGNOSIS_KEY, None)
            item.payload.pop(_REBASE_CONFLICT_AGENT_SUMMARY_KEY, None)
