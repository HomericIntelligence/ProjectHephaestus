...
```
# 🔍 Repository Audit Report (Full Coverage)
## {{derive from repo directory name, package.json name, or similar config}}
**Audit Date:** {{current_date}}
**Auditor:** Claude (Automated Repository Analysis — Full Coverage Mode)
**Coverage:** Every source file analyzed via per-section swarm agents
...
Follow these steps when performing the FULL COVERAGE audit:
  ...
  <step number="4">
    **SWARM DISPATCH — one Sonnet agent per audit section:**
    ...
    - This instruction: "READ EVERY FILE IN YOUR BUCKET. No sampling, no skipping. If a file is over 2000 lines, read it in chunks using Read with offset/limit. Report back: grade (letter + %), strengths, findings (with file:line), and principle compliance in the output_format structure."
    - This instruction: "If you cannot read a file (binary, locked, permission error), list it in a 'Coverage gaps' section with the reason."
  </step>

  <step number="5">
    After all 15 agents return their section assessments, calculate the overall score as a weighted average:
  ...
  <step number="6">
    Compile the consolidated issues list from all section agents. Sort critical issues first.
  </step>
  ...
  <step number="9">
    **COVERAGE VERIFICATION:**
    ...
    - Did any agent report files they could not read?
    - List all coverage gaps explicitly in the Coverage Report section.
    If a section agent did not return, re-dispatch it before finalizing the report.
  </step>
</analysis_instructions>
...
- **Full coverage guarantee:** The value of this variant over `repo-analyze` is that no file is skipped. If an agent reports it sampled or skipped files, send it back and require full coverage.
</important_notes>
